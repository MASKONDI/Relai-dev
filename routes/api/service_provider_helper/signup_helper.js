...
module.exports.updatePortpofolio = function (req) {
    return new Promise(async function (resolve, reject) {
        if (req.files.length != 0) {
            let ext_type = '';
            var ext = ''
            await req.files.forEach(async function(element,i){
                ...
                var resp = await ServiceProviderPortfolioSchema.updateMany(obj).where({_id:req.body.portpolio_img_id[i]})
                if(resp){
                    resolve(true)
                }
                // ServiceProviderPortfolioSchema.create(obj, async (err, item) => {
                ...
                //     }
                //     else {
                //         var resp = await ServiceProviderPortfolioSchema.updateMany(obj).where({_id:req.body.portpolio_img_id})
                //         resolve(true)
